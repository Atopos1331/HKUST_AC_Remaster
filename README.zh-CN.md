...
- Playwright + TOTP 自动登录 HKUST 空调门户
- 通过温湿度传感器模块读取室内温湿度
- 温控模式与定时模式两套控制策略
...
powers/
  ...
  io/
    ...
    default_thermometer.py 默认温湿度模块
    local_thermometer.example.py 本地私有覆盖示例
  services/
...
项目现在内置了一个默认温湿度模块。它会返回固定温度和湿度，这样即使没有真实传感器，整套控制逻辑也可以直接运行。

当前温度计入口拆分为：

- 抽象接口：`powers/io/thermometer.py`
- 默认回退实现：`powers/io/default_thermometer.py`
- 本地私有覆盖示例：`powers/io/local_thermometer.example.py`
...
默认入口 `get_thermometer()` 现在会优先尝试导入 `powers.io.local_thermometer`；如果该文件不存在，就回退到 `powers/io/default_thermometer.py`，并通过 `get_climate()` 返回 `IndoorClimateReading(temperature, humidity)`。

### 你可以怎么适配自己的设备

有三种典型方式：

1. 直接使用内置默认模块，返回固定温度和湿度。
2. 在本地创建 `powers/io/local_thermometer.py`，按现有 `Thermometer` 抽象写你自己的实现。
3. 如果你想自己控制实例化过程，也可以在 `powers/io/local_thermometer.py` 里暴露 `get_thermometer()` 工厂函数。
